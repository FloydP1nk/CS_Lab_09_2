...
//                 await client.GetAsync($"data/2.5/weather?lat={lat}&lon={lon}&appid=0a184e806f68bd923ca11cf17061f1f8");
                ...
                using (StreamReader reader = new(fs))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        else if (list.Count() == 3)
                        {
                            ...
                            cityLocation[list[0]] = new(Convert.ToDouble(list[1],numberFormatInfo), Convert.ToDouble(list[2],numberFormatInfo));
                            // cityLocation[list[0]] = new(double.Parse(list[1], CultureInfo.InvariantCulture),
                                // double.Parse(list[2], CultureInfo.InvariantCulture));
